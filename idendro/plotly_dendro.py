...
class PlotlyFeatures:
    def to_plotly(
        ...
        layout["xaxis"]["range"] = (
            xcoords.min()
            - (xcoords.max() - xcoords.min()) * 0.05 * (orientation != "right"),
    ...
    def get_point_traces(
        self, orientation, point_trace_kwargs, point_label_func, point_hover_func
    ):
        ...
        if point_label_func == "cluster_labels":
            point_label_func = (
                lambda x: "" if x["type"] != "cluster" else f"<b>{x['cluster_id']}</b>"
            )
        ...
        for (x, y), point in points.items():
            ...
            p = dict(
                ...
                marker=dict(
                    ...
                    size=14,
                    line=dict(width=2, color=to_hex(edgecolor)),
                ),
            ...
            if isinstance(hovertext, collections.abc.Mapping):
                p["hovertext"] = "<br>".join(
                    [f"<b>{k}</b> : {v}" for k, v in hovertext.items()]
                )
            ...
            for key, val in used_point_kwargs.items():
                if isinstance(val, collections.abc.Mapping) and key in p:
                    p[key].update(val)
                else:
                    p[key] = val
